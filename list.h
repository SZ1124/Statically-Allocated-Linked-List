...
struct Node_s {
    ...
    struct Node_s* prev;
    struct Node_s* next;
    //I tried to include array index, but next available node is nicer that it chains up with one another with current available node
    struct Node_s* nextAvailNode;
};
...
struct List_s{
    // TODO: You should change this!
    //size for the items in the list which can't exceed LIST_MAX_NUM_NODES
    int size;
    struct Node_s* head;
    struct Node_s* tail;
    struct Node_s* curr;
    //Same with Node struct's next available
    struct List_s* nextAvailList;
    //Instead of putting the List Out Of Bound state in a specific node that represent as current node,
    //I just put a enum out of bond state for the list
